...
int main() {
    ...
    std::filesystem::path vertex_shader_path = shaderSourcePath / "ex_3/vertex.glsl";
    std::filesystem::path fragment_shader_path = shaderSourcePath / "ex_3/fragment_debug.glsl";
    ...
    std::filesystem::path imagePath = assetsSourcePath / "woman2_lowres.jpg";
    int x,y,n;
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        static int normFactor = 4;
        static float bezel_width = 30;
        static float thickness = 1050;
        static float refractive_index = 1.5;
        static float roundedness = 100;
        static float shape_x = 450;
        static float shape_y = 400;
        static float shape_width = 300;
        static float shape_height = 200;

        ImGui::Checkbox("Debug Mode", &debugMode);

        ImGui::SliderInt("Norm Factor", &normFactor, 1, 10);

        ImGui::SliderFloat("Bezel Width", &bezel_width, 5, thickness / 2.0);
        ImGui::SliderFloat("Glass Thickness", &thickness, 100, 2000);
        ImGui::SliderFloat("Refractive Index", &refractive_index, 1.0, 5.0);
        ImGui::SliderFloat("Corner Radius", &roundedness, 10, 200);
        ImGui::SliderFloat("Width", &shape_width, 0, fb_width);
        ImGui::SliderFloat("Height", &shape_height, 0, fb_height);
        ...
        fb_glass.resize(fb_width, fb_height);
        fb_bg.resize(fb_width, fb_height);
        ...
        model = glm::translate(model, glm::vec3(shape_x, shape_y, 0.0f));
        model = glm::scale(model, glm::vec3(shape_width / 2.0, shape_height / 2.0, 1));
        glm::mat4 projection;
        projection = glm::ortho(0.0f, (float)fb_width, (float)fb_height, 0.0f, -1.0f, 1.0f);
        ...
        shaderProgram.setVec2("uHalfSize", glm::vec2(shape_width / 2.0, shape_height / 2.0));
        // bezel_width / uGlassThi
        shaderProgram.setBool("uDebugMode", debugMode);
        shaderProgram.setInt("uNormFactor", normFactor);
        shaderProgram.setFloat("bezel_width", bezel_width);
        shaderProgram.setFloat("uGlassThickness", thickness);
        shaderProgram.setFloat("refractive_index", refractive_index);
        shaderProgram.setFloat("uCornerRadius", roundedness);
